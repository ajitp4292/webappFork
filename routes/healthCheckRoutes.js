...
router.get('/', async (req, res) => {
  ...
  /*if (req.headers.authorization) {
    helper.logger.error(
      'Authorization header not required available for all users. - '
    );
    res.status(400).json({
    ...
    helper.logger.info('GET-healthz -Database Health Check Success. ');
    helper.statsdClient.increment('healthz_counter');

    res.status(200).end();
  } else if (!dbConnection) {
    helper.logger.info('GET-healthz -Database Health Check Fail. ');
